...
void UARTputc(char myChar)
{
	...
	while( (USART2 -> ISR & USART_ISR_TXE) == 0 );  // Wait till TXE flag is set
	USART2 -> TDR = (uint8_t) myChar;  						  // Then, cast the character to 8-bit binary and transmit!
}
...
char UARTgetc(void)
{
	...
	while( (USART2 -> ISR & USART_ISR_RXNE) == 0 );  //Wait until RXNE flag is set - this is a BLOCKING method - bad?
